...
本项目是从零实现 Harness 的命令行编程智能体，使用 DeepSeek V4 Flash Chat Completions 与原生 Tool Calling，不依赖 Agent 框架或服务端文件/执行工具。安装 Python 3.11+ 后执行 python -m pip install -e ".[dev]"。推荐通过 DEEPSEEK_API_KEY 环境变量配置密钥；本地也可使用已被 gitignore 排除的 api.txt。运行：python -m coding_agent --workspace 项目目录 "Fix the failing tests and verify the result."
...
Verification Guard 会在文件修改后检查是否执行有效验证；若模型过早结束，Harness 会提醒其运行测试、构建或程序。Workspace 统一限制文件路径和命令 cwd，隐藏 api.txt、.env 等凭据；子进程环境过滤常见密钥变量，命令采用 shell=False、超时和输出截断。这些是基础防护，不是真正 Sandbox。
